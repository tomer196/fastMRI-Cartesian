...
def evaluate(wl=False,device=1):
    args = create_arg_parser(device).parse_args()
    if wl:
        args.predictions_path=pathlib.Path(f'{device}rec_with')
...
def create_arg_parser(device):
    ...
    parser.add_argument('--target-path', type=pathlib.Path, default='/home/aditomer/Datasets/singlecoil_val',
                        help='Path to the ground truth data')
    parser.add_argument('--predictions-path', type=pathlib.Path, default=f'{device}rec_without',
    ...
    parser.add_argument('--acceleration', choices=[4, 8], default=[4],
